...
const Joke = (props) => {
      ...
      setJokes((prevJokes) => ({
        ...
        results: prevJokes.results.map((joke) => {
          return joke.id === id
            ? {
                ...joke,
                rating_count: joke.rating_count + 1,
                rating_id: data.id,
              }
            : joke;
        }),
  ...
  const editRating = async (rate) => {
    const dividedRating = rate / 20;
    try {
      await axiosRes.put(`/ratings/${rating_id}`, {
        joke: id,
        rating: dividedRating,
      });
    ...
    <Card className={styles.Joke}>
      <Card.Body>
        ...
        {currentUser && !is_owner && rating_id && (
          ...
          >
            <i onClick={deleteRating} className="fa-solid fa-eraser" />
          </OverlayTrigger>
        )}
        {alertMessage && (
          <Alert className={`${AlertStyles.alert} ${AlertStyles.alertSmall}`}>
            {alertMessage}
          </Alert>
        )}
      </Card.Body>
